...
static int fts_report_touch(struct ts_event *data)
{
    ...
    for (i = 0; i < data->touch_point; i++)
    {
        ...
        if (data->au8_touch_event[i]== 0 || data->au8_touch_event[i] == 2)
        {
			...
			touch_x = data->au16_x[i];
			touch_y = data->au16_y[i];

#if defined(TPD_LEFT_RIGHT_SWAP)
			tpd_swap_left_right(&touch_x, &touch_y);
#endif
#if defined(TPD_UP_DOWN_SWAP)
			tpd_swap_up_down(&touch_x, &touch_y);
#endif
#if defined(TPD_X_Y_SWAP)
			tpd_swap_x_y(&touch_x, &touch_y);
#endif
            input_report_abs(tpd->dev, ABS_MT_POSITION_X,touch_x);
            input_report_abs(tpd->dev, ABS_MT_POSITION_Y,touch_y);
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(retval < 0)
	{
		FTS_DEBUG("[TPD]fts chip probe fail\n");
		goto __probe_fail;
	}
