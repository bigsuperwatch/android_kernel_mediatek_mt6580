...
#ifdef CONFIG_WIITE_AMBIENT_DISPLAY_SCREEN
extern unsigned int idle_clock_mode;
extern unsigned int wiite_change_lcm_bl;
static unsigned int last_mapped_level  = 0;
#endif
// ---------------------------------------------------------------------------
//  Local Constants
// ---------------------------------------------------------------------------
...
static struct LCM_setting_table lcm_part_refresh_mode_setting[] = 
{
	// set Partial Area start row and end row
	//{0x30, 4, {0x00,0x00,0x00,0x01}},

	// Partial Mode On
	//{0x12, 1, {0x00}},
	{0x2A, 4, {0x00,0x10,0x00,0x20}},
	{0x2B, 4, {0x00,0x10,0x00,0x20}},
	{REGFLAG_END_OF_TABLE,99, {}}
};

static struct LCM_setting_table lcm_part_normal_mode_setting[] = 
{
	// normal Mode On
	//{0x13, 1, {0x00}},
	//{REGFLAG_DELAY, 120, {}},
	{0x2A, 4, {0x00,0x00,0x01,0x8F}},
...
void wiite_set_partial_mode(int enable)
{
	if(enable)
		push_table(lcm_part_refresh_mode_setting, sizeof(lcm_part_refresh_mode_setting) / sizeof(struct LCM_setting_table), 1);
	else
		push_table(lcm_part_normal_mode_setting, sizeof(lcm_part_normal_mode_setting) / sizeof(struct LCM_setting_table), 1);
}

void wiite_enter_idle_mode(int enable)
{
	if(enable)
...
void wiite_set_backlight(void)
{
	lcm_setbacklight(NULL,last_mapped_level);
}
#endif
...
{
#ifdef CONFIG_WIITE_AMBIENT_DISPLAY_SCREEN
	printk("RM67160 lcm_suspend for ambient screen,idle_clock_mode=%d\n",idle_clock_mode);
...
{
#ifdef CONFIG_WIITE_AMBIENT_DISPLAY_SCREEN
	printk("RM67160 lcm_resume for ambient screen,idle_clock_mode=%d\n",idle_clock_mode);
	if(idle_clock_mode == 1)
	{
		push_table(lcm_clk_sleep_mode_out_setting, sizeof(lcm_clk_sleep_mode_out_setting) / sizeof(struct LCM_setting_table), 1);
	}
...
static void lcm_setbacklight(void* handle,unsigned int level)
{
	...
	printk("rm67160 lcm_setbacklight and level is:(%d)\n",level);
	...
	if(level >0) 
	{
		...
		if(wiite_change_lcm_bl)
		{
			lcm_backlight_level_setting[0].para_list[0] = last_mapped_level;
			LCM_PRINT("rm67160 1 lcm_setbacklight setting level is %d,mapped_level=%d,last_mapped_level=%d\r\n", level,mapped_level,last_mapped_level);
	...
	else
	{
		   ...
		   if(idle_clock_mode == 1)
			...
			LCM_PRINT("rm67160 0 lcm_setbacklight setting level is %d,mapped_level=%d,last_mapped_level=%d\r\n", level,mapped_level,last_mapped_level);
		#else
			mapped_level = 0;
			LCM_PRINT("rm67160 0 lcm_setbacklight setting level is %d,mapped_level=%d \r\n", level,mapped_level);
		#endif
		// Refresh value of backlight level.
		lcm_backlight_level_setting[0].para_list[0] = mapped_level;
		push_table(lcm_backlight_level_setting, sizeof(lcm_backlight_level_setting) / sizeof(struct LCM_setting_table), 1);
...
static void lcm_setbacklight_no_cmdq(unsigned int level)
{
	unsigned int default_level = 45;
	unsigned int mapped_level = 0;
	printk("rm67160 lcm_setbacklight and level is:(%d)\n",level);
...
LCM_DRIVER rm67160_400x400_dsi_cmd_oled_drv = 
{
	...
	.set_backlight	= lcm_setbacklight_no_cmdq,
